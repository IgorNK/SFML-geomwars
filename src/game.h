...
class Game {
  ...
  const float read_config_f(const std::string & header, const std::string & value);
  const std::string read_config_s(const std::string & header, const std::string & value);
  void write_config(const std::string & header, const std::string & item, const std::string & value);
...
public:
	...
	void spawn_enemy();
	void spawn_boss();  
	const std::shared_ptr<Entity> spawn_laser(const CTransform & parent_transform, 
		const Vec2 & offset,
		const float length, 
	...
	void spawn_text_enemies(const std::string & text, const Vec2 & position, const int font_size, const sf::Color & font_color, const int spacing);
	void shoot();
